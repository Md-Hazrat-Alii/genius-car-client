...
const Checkout = () => {
    ...
    const handPlaceOrder = event => {
        event.preventDefault();
        const form = event.target;
        const name = `${form.firstName.value} ${form.lastName.value}`;
        const email = user?.email || 'unregistered';
        const phone = form.message.value
    ...
    return (
        <div>
            <form onSubmit={handPlaceOrder}>
                ...
                <input className='btn bg-blue-300' type="submit" value="Place Your Order" />
            </form>
        </div>
    );
};
